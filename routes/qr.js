...
const { processMessageQr } = require("../helper/addon/qr/processThings.js");
  ...
  setImmediate(async () => {
    try {
      const baileysMsg = {
        ...
        message: sendMsg.message || msgContent,
        messageTimestamp: Math.floor(Date.now() / 1000),
        pushName: user.name || user.email || "",
      };
      await processMessageQr({
        message: baileysMsg,
      ...
      console.log("Error saving sent message to inbox:", e.message);
    }
  });
